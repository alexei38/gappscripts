...
function updateRow(orig_ss, ss, values){
  ...
  // values = [timestamp, login, id, aliance, t4, t5, death, honor_main, honor_farms]
  // поля считаются с 0
  ...
  var t5 = values[5];
  var death = values[6];
  var honor_m = values[7];
  var honor_f = values[8];
  ...
  if(row_id){
    ...
    setNewData(ss, row_id, t4, t5, death, honor_m, honor_f)
...
function setNewData(ss, row_id, t4, t5, death, honor_m, honor_f){
  ...
  t4_kills = ss.getRange(row_id, t4_kills_idx);
  t4_kills.setValue(t4);

  t5_kills = ss.getRange(row_id, t5_kills_idx)
  t5_kills.setValue(t5)

  death_col = ss.getRange(row_id, death_idx);
  death_col.setValue(death);
